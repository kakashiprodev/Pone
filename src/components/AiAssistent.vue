        ...
        <Card class="w-12 mt-3 m-auto">
            <template #content>
                ...
                <div v-if="chatMessages.length === 2 && !loading">
                    <Button v-for="preset in  presets " :label="preset" @click="chat(preset)" class="w-full mt-2" />
                </div>
                <!-- Normal text input -->
                <div v-if="!showSpecialInput && !loading && !showEquivalentList && !finished" class="mt-4">
                ...
                <div v-if="showSpecialInput && !loading" class="mt-2">
                    ...
                    <InputNumber v-if="userInputType === 'number'" v-model="userInputValueNumber" class="w-full mt-2" />
                    <InputText v-if="userInputType === 'string'" v-model="userInputValueText" class="w-full mt-2" />
                </div>
                <!-- Equivalent list -->
                <SmartEquivalentList v-if="showEquivalentList && !loading" v-model="userInputEquivalent" />
                <!-- Abschicken Button-->
                <Button label="Abschicken" @click="chat()" class="mt-2 w-full" v-show="!loading && !finished" />
                <!-- Reset Button-->
                <Button label="Zurücksetzen" @click="reset()" class="mt-2 w-full" v-if="finished" />
            </template>
        </Card>
    </div>
</template>
...
const chatMessages: Ref<Message[]> = ref([
    ...
    { role: "system", content: "Hallo, ich bin dein persönlicher Eingabe Assistent.\nIch werde dir helfen deine Daten einzugeben.\n\nWas möchtest du eingeben?" }
]);
...
const chat = async (text?: string) => {
    ...
    if (text == null && showEquivalentList.value) {
        ...
        // find equivalent
        const e = globalStore.equivalentDict[userInputEquivalent.value ?? ''] ?? null;
        if (e != null) {
    ...
    try {
        ...
        if (message.necessary_input != null) {
            ...
            if (message.necessary_input.type === "number") {
                userInputValueNumber.value = message.necessary_input.default_value === "" ? null : Number(message.necessary_input.default_value);
            } else {
        ...
        if (message.necessary_action != null && message.necessary_action !== '') {
            if (message.necessary_action === "do_input") {
                ...
                chatMessages.value.push({ role: "system", content: JSON.stringify(message.values, null, 2) });
                chatMessages.value.push({ role: "system", content: "Eingabe abgeschlossen. Danke dass ich Sie unterstützen durfte." });
                finished.value = true;
