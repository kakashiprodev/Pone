/**
 * CSRD ESRS Topics for sustainability reporting
 * This will a list of all needed topics for the ESRS resulting in the CSRD Report
 *
 * ESRS E1: Klimaschutz (Mitigation) und Anpassung an den Klimawandel (Adaption)
 * ESRS E2: Umweltverschmutzung
 * ESRS E3: Wasser- und Meeresressourcen
 * ESRS E4: Biodiversität und Ökosysteme
 * ESRS E5: Ressourcennutzung & Kreislaufwirtschaft
 */

/*
Auszug dazu: https://www.umweltbundesamt.de/umweltberichterstattung-berichtsstandards#internationale-nachhaltigkeitsberichtsstandards
ESRS 1 enthält allgemeine Anforderungen an die Nachhaltigkeitsberichterstattung eines Unternehmens, definiert die Anwendungsregeln für die Standards, Struktur der Berichte und Berichtsprinzipien. Darunter fallen auch Vorgaben zur Wesentlichkeitsanalyse und dem Prinzip der doppelten Wesentlichkeit, der Einbeziehung von Stakeholdern bei der Bestimmung wesentlicher Themen, Erläuterungen zur Verbindung von unternehmerischen Sorgfaltspflichten mit den Berichtsanforderungen sowie Prinzipien zur Berichterstattung über Liefer- und Wertschöpfungsketten. Gemäß der vorgesehenen Anwendungsregeln können Unternehmen ganze Berichtsstandards auslassen, sofern sie die darin abgedeckten Nachhaltigkeitsthemen als unwesentlich bestimmen. Für als wesentlich deklarierte Themen sind die jeweiligen Berichtsstandards anzuwenden und gegebenenfalls auch darüberhinausgehende Informationen offenzulegen. Bestimmte Kennzahlen oder Datenpunkte können auch für wesentliche Themen ausgelassen werden, falls diese im Einzelfall unwesentliche Informationen erzeugen würden. Verpflichtend von allen Unternehmen im Anwendungsbereich der CSRD anzuwenden, sind die Anforderungen des Berichtsstandards ESRS 2 „General Disclosures“.

ESRS 2 enthält themenübergreifende Berichtsanforderungen.
Offenzulegen sind unter anderem Governance-Strukturen sowie das Verhältnis von Strategie und Geschäftsmodell hinsichtlich Nachhaltigkeitsthemen.
Zudem ist der Prozess zur Ermittlung wesentlicher nachhaltigkeitsbezogener Auswirkungen,
Risiken und Chancen (sog. „impacts, risks and opportunities“ – IRO) zu beschreiben. ESRS 2 definiert darüber hinaus Mindestberichtsanforderungen (sog. „Minimum Disclosure Requirements“) zu Unternehmenspolitiken, Maßnahmen, Kennzahlen und Zielen, die Grundlage für die entsprechende Berichterstattung anhand der themenbezogenen Standards sind. ESRS 2 folgt sinngemäß einer von der “Task Force on climate-related financial disclosures” (TCFD) empfohlenen Struktur, wodurch Kompatibilität internationalen Berichtsstandards hergestellt werden soll. Diese Struktur gilt auch für die themenbezogenen ESRS.

Im Umweltbereich sind fünf themenbezogene Berichtsstandards vorgesehen:

ESRS E1 enthält Berichtsanforderungen zu ⁠Klimaschutz⁠, Klimaanpassung und Energie.

ESRS E2 enthält Berichtsanforderungen zur Umweltverschmutzung, insbesondere zur Verschmutzung von Luft, Wasser und Boden sowie besorgniserregenden Stoffen.

ESRS E3 enthält Berichtsanforderungen zu Wasserverbräuchen und der Nutzung mariner Ressourcen.

ESRS E4 enthält Berichtsanforderungen zu ⁠Biodiversität⁠ und Ökosystemen, unter anderem zu ⁠Landnutzung⁠ und ⁠Landnutzungsänderung⁠, inklusive Entwaldung, invasiven Arten und weiteren Treibern für Biodiversitätsverlust sowie zum Zustand von Arten und Ökosystemen.

ESRS E5 enthält Berichtsanforderungen zu Ressourcennutzung und Circular Economy, insbesondere zu Materialien und deren zirkulären Verwendung, Produkten des Unternehmens und zirkulären Verwendungsmöglichkeiten sowie Abfällen.

Branchenspezifische Berichtsstandards
Zusätzlich zu den Berichtsstandards, die sich an alle Unternehmen im Anwendungsbereich der CSRD richten, sollen in Zukunft auch konkretisierende branchenspezifische Standards verabschiedet werden. EFRAG hat mit der Erarbeitung der Berichtsstandards für erste Branchen begonnen.

Berichtsstandards für kleine und mittlere Unternehmen
In den Anwendungsbereich der CSRD fallen ab dem Jahr 2026 kapitalmarktorientierte kleine und mittlere Unternehmen. Damit diese nicht unangemessen von den Berichtspflichten belastet werden, sollen sie vereinfachte Berichtsstandards nutzen und die Anwendung bis 2028 aufschieben können. Diese Berichtsstandards werden von EFRAG derzeit erarbeitet und sollen von der Europäischen Kommission bis zum 30.06.2024 als delegierter Rechtsakt verabschiedet werden. Die Inhalte der Berichtsstandards begrenzen zudem die Informationen, die große berichtspflichtige Unternehmen von kleinen und mittleren Unternehmen in ihrer Wertschöpfungskette einholen dürfen (sog. „value chain cap“).

Für kleine und mittlere Unternehmen, die nicht in den Anwendungsbereich der CSRD fallen, aber aufgrund ihrer Geschäftsbeziehungen mit größeren Unternehmen Anfragen zu Nachhaltigkeitsinformationen erhalten können, wird ein freiwilliger, stark vereinfachter und modular aufgebauter Berichtsstandard von EFRAG entwickelt.

Erarbeitungsprozess der Standards
Bei der Erarbeitung der EU-Nachhaltigkeitsberichtsstandards kommt der Europäischen Beratungsgruppe für Rechnungslegung (European Financial Reporting Advisory Group – EFRAG) eine zentrale Rolle zu. EFRAG’s Arbeit konzentrierte sich ursprünglich auf die Mitarbeit an und Bewertung von internationalen Rechnungslegungsstandards. Im Zuge der Novelle der CSR-Richtlinie erhielt EFRAG seitens der Europäischen Kommission (Generaldirektion Finanzstabilität, Finanzdienstleistungen und Kapitalmarktunion – FISMA) ein Mandat, vorbereitende Arbeiten für die EU-Nachhaltigkeitsberichtsstandards durchzuführen und ein ordentliches Verfahren (sog. „due process“) zur Entwicklung dieser Standards einzuführen.

EFRAG hat im März 2022 zwei Gremien eingerichtet, die „Sustainability Reporting Technical Expert Group“ (SR TEG) und das „Sustainability Reporting Board“ (SRB), die für den Standardentwicklungsprozess verantwortlich sind. Beide Gremien sind mit Vertreter*innen verschiedener Stakeholdergruppen besetzt. In der Praxis werden erste Entwürfe für Standards durch das EFRAG-Sekretariat vorbereitet, von der SR TEG diskutiert und abgeändert und anschließend im SRB diskutiert und beschlossen. Für jeden Standard sehen die Verfahrensregeln auch eine öffentliche Konsultation vor. Abweichend davon wurden die vorbereitenden Arbeiten für das Set 1 an Berichtsstandards von einer Projekt-Task Force übernommen, die im Juni 2021 parallel zu den Verhandlungen zur Überarbeitung der CSR-Richtlinie ihr Arbeit aufnahm. Das Arbeitsergebnis der Task Force wurde im April 2022 an die EFRAG-Gremien übergeben und zur öffentlichen Konsultation gestellt. Das SRB hat mit Unterstützung der SR TEG daraufhin die Entwürfe finalisiert und am 23.11.2022 an die Europäische Kommission übergeben. Die Europäische Kommission hat die Standards nach Konsultation verschiedener EU-Institutionen, der EU-Mitgliedstaaten und der Öffentlichkeit noch einmal angepasst und am 31. Juli 2023 verabschiedet. In diesem letzten Schritt wurden zahlreiche Erleichterungen für die berichtspflichtigen Unternehmen eingeführt.

Internationale Nachhaltigkeitsberichtsstandards
Auch auf internationaler Ebene gibt es Bestrebungen zu einheitlichen Nachhaltigkeitsberichtsstandards für Unternehmen.

Die ehemalig stark fragmentierte Landschaft an Nachhaltigkeitsberichtsstandards für Unternehmen wurde durch die Gründung des „International Sustainability Standards Board“ (ISSB) im November 2021 bei der „International Financial Reporting Standards“ (IFRS)-Stiftung weitgehend konsolidiert. Die ehemaligen Standardsetzer „Sustainability Accounting Standards Board“ (SASB), „International Integrated Reporting Council“ (IIRC) und das „Climate Disclosure Standards Board“ (CDSB) sind in der IFRS-Stiftung aufgegangen. Das ISSB hat zwei Standards für die Berichterstattung nachhaltigkeitsbezogener Finanzinformationen entwickelt, die sich insbesondere an die Kapitalmärkte richten. Der Standard IFRS S1 enthält generische und themenübergreifende Berichtsanforderungen und der Standard IFRS S2 Anforderungen zur Offenlegung klimabezogener Finanzinformationen. In Zukunft sollen Berichtsstandards zu weiteren Themen folgen. Die Anwendung von IFRS S1 und S2 ist freiwillig.

Demgegenüber besteht die Global Reporting Initiative (GRI) fort. Die GRI entwickelt bereits seit 25 Jahren Nachhaltigkeitsberichtsstandards für Unternehmen und andere Organisationen. Im Fokus der GRI-Standards stehen die Auswirkungen einer Organisation auf Umwelt, Menschen und Wirtschaft. Die nach GRI berichteten Informationen sollen folglich an die Vielzahl von Stakeholdern eines Unternehmens gerichtet sein. Die Anwendung der GRI-Standards ist freiwillig. Die GRI-Standards sind ähnlich der neuen EU-Nachhaltigkeitsberichtsstandards aufgebaut und enthalten generische, themenbezogene und branchenspezifische Berichtsanforderungen. Die IFRS-Stiftung und GRI haben im März 2022 eine Absichtserklärung unterzeichnet ihre Arbeiten zu koordinieren.

Die europäischen Nachhaltigkeitsberichtsstandards sind kompatibel mit den Standards der IFRS-Stiftung und denen der GRI. Die ESRS folgen dabei dem Prinzip der „doppelten Wesentlichkeit“. Dies beinhaltet sowohl eine Berichterstattung über die aus finanzieller Sicht wesentlichen Nachhaltigkeitsinformationen (Fokus der IFRS-Standards) als auch über die wesentlichen Auswirkungen eines Unternehmens auf Umwelt, Menschenrechte und weitere Sozialbelange (Fokus der GRI-Standards).

ESRS 1
Der ESRS 1 beschreibt die allgemeinen Anforderungen an die Erstellung und Darstellung nachhaltigkeitsbezogener Informationen, wie etwa die zugrunde liegenden Konzepte und Prinzipien:

Das Prinzip der Doppelten Wesentlichkeit zieht sich durch die Standards. Bei der Bestimmung der Wesentlichkeit gilt es die zwei Perspektiven inside-out und outside-in einzunehmen. Es gilt eine Oder-Bedingung.
Weiterhin liegt die Sorgfaltspflicht für Nachhaltigkeitsbelange beim Unternehmen. Der Begriff wird auch in der europäischen Variante des Lieferkettensorgfaltspflichten-Gesetzes verwendet (Corporate Sustainability Due Dilligence Directive, CSDDD).
Im Sinne einer Zeitreihenbetrachtung sind sowohl historische als auch aktuelle und zukünftige Werte zu berichten. Damit soll die Entwicklung transparent gemacht werden.
Die Standards fordern Transparenz zu (Berechnungs-)Methoden, Kontext zur Einordnung. Die Prinzipien Vollständigkeit, Vergleichbarkeit und Überprüfbarkeit werden hier betont.
ESRS 2
Der ESRS 2 legt Angabepflichten fest, die das Unternehmen auf einer allgemeinen Ebene in Bezug auf alle wesentlichen Nachhaltigkeitsaspekte vorlegen muss. Darunter die folgenden:

Kontext
Hintergrundinformation zum Nachhaltigkeitsbericht
wichtige Kontextinformationen
Governance
Zusammensetzung und Nachhaltigkeitsexpertise der Verwaltungs-, Management- und Aufsichtsorgane
Betrachtung von Nachhaltigkeitsbelangen durch die Aufsichtsorgane
Integration von nachhaltigkeitsbezogener Leistung in Anreizsysteme
Erklärung zu Due-Diligence für Nachhaltigkeitsbelange
Risikomanagement und interne Kontrolle der Nachhaltigkeitsberichterstattung
Strategie
Marktposition, Strategie, Geschäftsmodell und Wertschöpfungskette
Interessen und Ansichten der Stakeholder (inkl. Zweck der Stakeholder-Beteiligung und Berücksichtigung der Ergebnisse in Wesentlichkeitsanalyse und/oder Due Diligence sowie in Unternehmensstrategie und/oder Geschäftsmodell)
Wesentliche Auswirkungen, Risiken und Chancen und deren Zusammenspiel mit Strategie und Geschäftsmodell
Management
Offenlegungen zur Wesentlichkeitsanalyse (Vorgehen)
Berücksichtigte ESRS-Offenlegungsanforderungen
*/

/**
 * One Topic will be one of the ESRS Topics but in detail. So one ESRS Report Topic will have multiple Topics here
 */

export interface InterviewTopic {
  id: string;
  name: string;
  description: string;
}

export interface EsrsTopic {
  id: string;
  esrsGroup: string;
  name: string;
  tags: string[];
  descriptionForInterviewModerator: string;
  interviewTopics: InterviewTopic[];
}

export interface TopicState {
  [idOfTopic: string]: {
    started: boolean;
    done: boolean;
  };
}

export interface UsersTopicAnswer {
  id: string;
  report: string; // reference on table report
  topicId: string;
  topicHeader: string;
  summary: string;
  collectedInformation: string[];
  isDone: boolean;
}

export interface UsersTopicAnswerDict {
  [idOfTopic: string]: UsersTopicAnswer[];
}

export const esrsTopics: { [key: string]: EsrsTopic[] } = {
  'de':
    [
      // ESRS E1 - Klimaschutz (Mitigation & Adaption)
      {
        id: 'E1-01',
        esrsGroup: 'ESRS E1',
        name: 'Klimaschutz (Minderung)',
        tags: ['Klimaschutz', 'Minderung'],
        descriptionForInterviewModerator:
          'Diskutieren Sie Initiativen und Strategien zur Reduzierung des CO2-Fußabdrucks und der Treibhausgasemissionen.',
        interviewTopics: [
          {
            id: 'E1-01-01',
            name: 'Ziele zur Emissionsreduktion',
            description:
              'Welche aktuellen Ziele zur Emissionsreduktion gibt es und welche Strategien sind zur Erreichung dieser Ziele vorhanden?',
          },
          {
            id: 'E1-01-02',
            name: 'Nutzung erneuerbarer Energien',
            description:
              'Welcher Prozentsatz Ihrer Energie stammt aus erneuerbaren Quellen und welche Pläne gibt es, diesen Anteil zu erhöhen?',
          },
          {
            id: 'E1-01-03',
            name: 'Maßnahmen zur Energieeffizienz',
            description:
              'Welche Verbesserungen der Energieeffizienz wurden implementiert oder sind in den Betriebsprozessen geplant?',
          },
        ],
      },
      {
        id: 'E1-02',
        esrsGroup: 'ESRS E1',
        name: 'Klimaschutz (Adaption)',
        tags: ['Klimaschutz', 'Adaption'],
        descriptionForInterviewModerator:
          'Erforschen Sie die Anpassungsstrategien des Unternehmens an die Auswirkungen des Klimawandels.',
        interviewTopics: [
          {
            id: 'E1-02-01',
            name: 'Ziele zur Adaption der Folgen des Klimawandels',
            description:
              'Welche aktuellen Ziele zur Anpassung an die Folgen des Klimawandels gibt es und welche Strategien sind zur Erreichung dieser Ziele vorhanden?',
          },
          {
            id: 'E1-02-02',
            name: 'Risikomanagement in Bezug auf Klimawandel',
            description:
              'Wie integriert das Unternehmen Risiken im Zusammenhang mit dem Klimawandel in sein Risikomanagement? Gibt es Notfallpläne oder Anpassungen im Geschäftsmodell?',
          },
          {
            id: 'E1-02-03',
            name: 'Bau- und Infrastrukturmaßnahmen zur Anpassung',
            description:
              'Welche baulichen Maßnahmen und Infrastrukturprojekte wurden implementiert, um sich an den Klimawandel anzupassen?',
          },
        ],
      },
      // ESRS E2 - Umweltverschmutzung
      {
        id: 'E2-01',
        esrsGroup: 'ESRS E2',
        name: 'Schadstoffkontrolle',
        tags: ['Umweltverschmutzung', 'Verschmutzung'],
        descriptionForInterviewModerator:
          'Erforschen Sie Maßnahmen zur Vermeidung, Reduzierung oder Bewältigung von Schadstoffemissionen in Luft, Wasser und Boden.',
        interviewTopics: [
          {
            id: 'E2-01-01',
            name: 'Luftqualitätsmanagement',
            description:
              'Welche Systeme sind vorhanden, um die Luftqualität zu überwachen und zu verwalten, und welche Ergebnisse wurden erzielt?',
          },
          {
            id: 'E2-01-02',
            name: 'Kontrolle der Wasserverschmutzung',
            description:
              'Beschreiben Sie die Abwasserbehandlungsprozesse und deren Übereinstimmung mit den gesetzlichen Standards.',
          },
          {
            id: 'E2-01-03',
            name: 'Abfallmanagement',
            description:
              'Wie verwaltet das Unternehmen gefährliche und ungefährliche Abfälle und welche Strategien zur Reduktion gibt es?',
          },
          {
            id: 'E2-01-04',
            name: 'Umgang mit gefährlichen Stoffen',
            description:
              'Welche Strategien und Maßnahmen wurden implementiert, um den Einsatz gefährlicher Stoffe zu minimieren und deren Entsorgung sicher zu gestalten?',
          },
        ],
      },
      // ESRS E3 - Wasser- und Meeresressourcen
      {
        id: 'E3-01',
        esrsGroup: 'ESRS E3',
        name: 'Wasser- und Meeresressourcenmanagement',
        tags: ['Wassermanagement', 'Meeresressourcen'],
        descriptionForInterviewModerator:
          'Diskutieren Sie die Praktiken des Unternehmens bezüglich der nachhaltigen Nutzung und des Schutzes von Wasser- und Meeresressourcen.',
        interviewTopics: [
          {
            id: 'E3-01-01',
            name: 'Effizienz der Wassernutzung',
            description:
              'Welche Maßnahmen werden ergriffen, um eine effiziente Wassernutzung in den Betrieben zu gewährleisten? Bitte geben Sie spezifische Beispiele an.',
          },
          {
            id: 'E3-01-02',
            name: 'Auswirkungen auf marine Ökosysteme',
            description:
              'Wie bewertet und mildert das Unternehmen seine Auswirkungen auf marine Ökosysteme?',
          },
          {
            id: 'E3-01-03',
            name: 'Maßnahmen zur Wasserreinhaltung',
            description:
              'Welche Technologien oder Praktiken setzt das Unternehmen ein, um Wasserverunreinigung zu verhindern oder zu minimieren?',
          },
          {
            id: 'E3-01-04',
            name: 'Wasserrückgewinnung und -recycling',
            description:
              'Welche Systeme zur Wasserrückgewinnung und zum Recycling werden in den Betriebsprozessen implementiert?',
          },
        ],
      },
      // ESRS E4 - Biodiversität und Ökosysteme
      {
        id: 'E4-01',
        esrsGroup: 'ESRS E4',
        name: 'Erhaltung der Biodiversität und der Ökosysteme',
        tags: ['Biodiversität', 'Ökosysteme'],
        descriptionForInterviewModerator:
          'Bewerten Sie die Bemühungen und Strategien des Unternehmens zum Schutz der Biodiversität und zur Erhaltung gesunder Ökosysteme.',
        interviewTopics: [
          {
            id: 'E4-01-01',
            name: 'Landnutzung und -management',
            description:
              'Wie verwaltet das Unternehmen seine Landnutzung, um den Erhalt der Biodiversität zu unterstützen?',
          },
          {
            id: 'E4-01-02',
            name: 'Management invasiver Arten',
            description:
              'Welche Maßnahmen werden ergriffen, um invasive Arten zu kontrollieren oder zu eradizieren, die die Ökosysteme beeinflussen, in denen das Unternehmen tätig ist?',
          },
          {
            id: 'E4-01-03',
            name: 'Naturschutzinitiativen',
            description:
              'An welchen spezifischen Naturschutzinitiativen beteiligt sich das Unternehmen oder führt diese durch?',
          },
          {
            id: 'E4-01-04',
            name: 'Förderung von Biodiversität in Betriebsabläufen',
            description:
              'Welche Maßnahmen werden unternommen, um die Biodiversität in und um Betriebsstätten aktiv zu fördern?',
          },
        ],
      },
      // ESRS E5 - Ressourcennutzung & Kreislaufwirtschaft
      {
        id: 'E5-01',
        esrsGroup: 'ESRS E5',
        name: 'Ressourcennutzung und Kreislaufwirtschaft',
        tags: ['Ressourcennutzung', 'Kreislaufwirtschaft'],
        descriptionForInterviewModerator:
          'Erforschen Sie, wie das Unternehmen die Prinzipien der Kreislaufwirtschaft in sein Geschäftsmodell und seine Betriebsabläufe integriert.',
        interviewTopics: [
          {
            id: 'E5-01-01',
            name: 'Materialrecycling',
            description:
              'Welche Prozesse gibt es für das Recycling der in der Produktion verwendeten Materialien?',
          },
          {
            id: 'E5-01-02',
            name: 'Produktlebenszyklus',
            description:
              'Wie gestaltet das Unternehmen seine Produkte, um deren Lebenszyklus zu verlängern oder das Recycling am Ende der Lebensdauer zu erleichtern?',
          },
          {
            id: 'E5-01-03',
            name: 'Strategien zur Abfallreduktion',
            description:
              'Welche Strategien setzt das Unternehmen ein, um Abfälle in seinen Betriebsabläufen zu reduzieren?',
          },
          {
            id: 'E5-01-04',
            name: 'Nutzung nachhaltiger Materialien',
            description:
              'Wie fördert das Unternehmen die Nutzung nachhaltiger Materialien in seinen Produkten und Prozessen?',
          },
        ],
      },
      // ESRS 2 - General Disclosures (Themenübergreifende Berichtsanforderungen)
      {
        id: 'ESRS2-01',
        esrsGroup: 'ESRS 2',
        name: 'Allgemeine Unternehmensinformationen',
        tags: ['Governance', 'Strategie'],
        descriptionForInterviewModerator:
          'Erfassen Sie allgemeine Informationen über die Governance-Strukturen und die strategische Ausrichtung des Unternehmens in Bezug auf Nachhaltigkeit.',
        interviewTopics: [
          {
            id: 'ESRS2-01-01',
            name: 'Governance-Struktur und -Kompetenz',
            description:
              'Beschreiben Sie die Zusammensetzung und Expertise der Governance-Gremien des Unternehmens im Hinblick auf Nachhaltigkeitsfragen.',
          },
          {
            id: 'ESRS2-01-02',
            name: 'Integration von Nachhaltigkeit in die Geschäftsstrategie',
            description:
              'Wie werden Nachhaltigkeitsüberlegungen in die Geschäftsstrategie und das Geschäftsmodell integriert?',
          },
          {
            id: 'ESRS2-01-03',
            name: 'Nachhaltigkeitsrisikomanagement',
            description:
              'Welche Prozesse hat das Unternehmen implementiert, um Nachhaltigkeitsrisiken zu identifizieren und zu managen?',
          },
        ],
      },
      {
        id: 'ESRS2-02',
        esrsGroup: 'ESRS 2',
        name: 'Wesentlichkeitsanalyse',
        tags: ['Wesentlichkeit', 'Stakeholder-Engagement'],
        descriptionForInterviewModerator:
          'Erforschen Sie den Prozess der Wesentlichkeitsanalyse und die Einbindung von Stakeholdern in diesen Prozess.',
        interviewTopics: [
          {
            id: 'ESRS2-02-01',
            name: 'Prozess der Wesentlichkeitsanalyse',
            description:
              'Wie führt das Unternehmen seine Wesentlichkeitsanalyse durch und welche Kriterien werden verwendet?',
          },
          {
            id: 'ESRS2-02-02',
            name: 'Stakeholder-Engagement',
            description:
              'Welche Stakeholder werden in den Prozess der Wesentlichkeitsanalyse einbezogen und wie wird deren Feedback berücksichtigt?',
          },
        ],
      },
      {
        id: 'ESRS2-03',
        esrsGroup: 'ESRS 2',
        name: 'Offenlegung von Unternehmenspolitiken und Zielen',
        tags: ['Politiken', 'Ziele'],
        descriptionForInterviewModerator:
          'Erfassen Sie Informationen über die unternehmensweiten Nachhaltigkeitspolitiken und -ziele.',
        interviewTopics: [
          {
            id: 'ESRS2-03-01',
            name: 'Unternehmenspolitiken zu Nachhaltigkeit',
            description:
              'Welche Nachhaltigkeitspolitiken hat das Unternehmen implementiert, und wie werden diese überwacht?',
          },
          {
            id: 'ESRS2-03-02',
            name: 'Nachhaltigkeitsziele und -kennzahlen',
            description:
              'Welche spezifischen Nachhaltigkeitsziele hat sich das Unternehmen gesetzt, und welche Kennzahlen werden zur Überwachung verwendet?',
          },
          {
            id: 'ESRS2-03-03',
            name: 'Berichterstattungspraktiken',
            description:
              'Wie stellt das Unternehmen die Transparenz und Vollständigkeit seiner Nachhaltigkeitsberichterstattung sicher?',
          },
        ],
      },
    ],
  'en': [
    // ESRS E1 - Climate Protection (Mitigation & Adaptation)
    {
      id: 'E1-01',
      esrsGroup: 'ESRS E1',
      name: 'Climate Protection (Mitigation)',
      tags: ['Climate Protection', 'Mitigation'],
      descriptionForInterviewModerator:
        'Discuss initiatives and strategies to reduce the carbon footprint and greenhouse gas emissions.',
      interviewTopics: [
        {
          id: 'E1-01-01',
          name: 'Emission Reduction Targets',
          description:
            'What current emission reduction targets are in place, and what strategies are there to achieve these targets?',
        },
        {
          id: 'E1-01-02',
          name: 'Use of Renewable Energy',
          description:
            'What percentage of your energy comes from renewable sources, and what plans are there to increase this share?',
        },
        {
          id: 'E1-01-03',
          name: 'Energy Efficiency Measures',
          description:
            'What energy efficiency improvements have been implemented or are planned in operational processes?',
        },
      ],
    },
    {
      id: 'E1-02',
      esrsGroup: 'ESRS E1',
      name: 'Climate Protection (Adaptation)',
      tags: ['Climate Protection', 'Adaptation'],
      descriptionForInterviewModerator:
        'Explore the company’s adaptation strategies to the impacts of climate change.',
      interviewTopics: [
        {
          id: 'E1-02-01',
          name: 'Climate Change Adaptation Goals',
          description:
            'What current goals exist for adapting to the impacts of climate change, and what strategies are there to achieve these goals?',
        },
        {
          id: 'E1-02-02',
          name: 'Risk Management Regarding Climate Change',
          description:
            'How does the company integrate climate change-related risks into its risk management? Are there contingency plans or adjustments to the business model?',
        },
        {
          id: 'E1-02-03',
          name: 'Construction and Infrastructure Adaptation Measures',
          description:
            'What construction measures and infrastructure projects have been implemented to adapt to climate change?',
        },
      ],
    },
    // ESRS E2 - Pollution
    {
      id: 'E2-01',
      esrsGroup: 'ESRS E2',
      name: 'Pollution Control',
      tags: ['Pollution', 'Contamination'],
      descriptionForInterviewModerator:
        'Explore measures to prevent, reduce, or manage pollutant emissions in air, water, and soil.',
      interviewTopics: [
        {
          id: 'E2-01-01',
          name: 'Air Quality Management',
          description:
            'What systems are in place to monitor and manage air quality, and what results have been achieved?',
        },
        {
          id: 'E2-01-02',
          name: 'Water Pollution Control',
          description:
            'Describe the wastewater treatment processes and their compliance with legal standards.',
        },
        {
          id: 'E2-01-03',
          name: 'Waste Management',
          description:
            'How does the company manage hazardous and non-hazardous waste, and what strategies exist for reduction?',
        },
        {
          id: 'E2-01-04',
          name: 'Handling of Hazardous Substances',
          description:
            'What strategies and measures have been implemented to minimize the use of hazardous substances and ensure safe disposal?',
        },
      ],
    },
    // ESRS E3 - Water and Marine Resources
    {
      id: 'E3-01',
      esrsGroup: 'ESRS E3',
      name: 'Water and Marine Resources Management',
      tags: ['Water Management', 'Marine Resources'],
      descriptionForInterviewModerator:
        'Discuss the company’s practices regarding the sustainable use and protection of water and marine resources.',
      interviewTopics: [
        {
          id: 'E3-01-01',
          name: 'Water Use Efficiency',
          description:
            'What measures are being taken to ensure efficient water use in operations? Please provide specific examples.',
        },
        {
          id: 'E3-01-02',
          name: 'Impact on Marine Ecosystems',
          description:
            'How does the company assess and mitigate its impact on marine ecosystems?',
        },
        {
          id: 'E3-01-03',
          name: 'Water Purity Measures',
          description:
            'What technologies or practices does the company use to prevent or minimize water pollution?',
        },
        {
          id: 'E3-01-04',
          name: 'Water Recovery and Recycling',
          description:
            'What systems for water recovery and recycling are implemented in the operational processes?',
        },
      ],
    },
    // ESRS E4 - Biodiversity and Ecosystems
    {
      id: 'E4-01',
      esrsGroup: 'ESRS E4',
      name: 'Biodiversity and Ecosystem Conservation',
      tags: ['Biodiversity', 'Ecosystems'],
      descriptionForInterviewModerator:
        'Evaluate the company’s efforts and strategies to protect biodiversity and preserve healthy ecosystems.',
      interviewTopics: [
        {
          id: 'E4-01-01',
          name: 'Land Use and Management',
          description:
            'How does the company manage its land use to support biodiversity conservation?',
        },
        {
          id: 'E4-01-02',
          name: 'Management of Invasive Species',
          description:
            'What measures are being taken to control or eradicate invasive species that affect the ecosystems where the company operates?',
        },
        {
          id: 'E4-01-03',
          name: 'Conservation Initiatives',
          description:
            'Which specific conservation initiatives is the company involved in or leading?',
        },
        {
          id: 'E4-01-04',
          name: 'Promoting Biodiversity in Operations',
          description:
            'What measures are being taken to actively promote biodiversity in and around operational sites?',
        },
      ],
    },
    // ESRS E5 - Resource Use & Circular Economy
    {
      id: 'E5-01',
      esrsGroup: 'ESRS E5',
      name: 'Resource Use and Circular Economy',
      tags: ['Resource Use', 'Circular Economy'],
      descriptionForInterviewModerator:
        'Explore how the company integrates the principles of the circular economy into its business model and operations.',
      interviewTopics: [
        {
          id: 'E5-01-01',
          name: 'Material Recycling',
          description:
            'What processes exist for recycling materials used in production?',
        },
        {
          id: 'E5-01-02',
          name: 'Product Lifecycle',
          description:
            'How does the company design its products to extend their lifecycle or facilitate recycling at the end of their life?',
        },
        {
          id: 'E5-01-03',
          name: 'Waste Reduction Strategies',
          description:
            'What strategies does the company use to reduce waste in its operations?',
        },
        {
          id: 'E5-01-04',
          name: 'Use of Sustainable Materials',
          description:
            'How does the company promote the use of sustainable materials in its products and processes?',
        },
      ],
    },
    // ESRS 2 - General Disclosures (Cross-Cutting Reporting Requirements)
    {
      id: 'ESRS2-01',
      esrsGroup: 'ESRS 2',
      name: 'General Corporate Information',
      tags: ['Governance', 'Strategy'],
      descriptionForInterviewModerator:
        'Collect general information about the company’s governance structures and strategic direction regarding sustainability.',
      interviewTopics: [
        {
          id: 'ESRS2-01-01',
          name: 'Governance Structure and Competence',
          description:
            'Describe the composition and expertise of the company’s governance bodies concerning sustainability issues.',
        },
        {
          id: 'ESRS2-01-02',
          name: 'Integration of Sustainability into Business Strategy',
          description:
            'How are sustainability considerations integrated into the business strategy and model?',
        },
        {
          id: 'ESRS2-01-03',
          name: 'Sustainability Risk Management',
          description:
            'What processes has the company implemented to identify and manage sustainability risks?',
        },
      ],
    },
    {
      id: 'ESRS2-02',
      esrsGroup: 'ESRS 2',
      name: 'Materiality Analysis',
      tags: ['Materiality', 'Stakeholder Engagement'],
      descriptionForInterviewModerator:
        'Explore the process of materiality analysis and stakeholder engagement in this process.',
      interviewTopics: [
        {
          id: 'ESRS2-02-01',
          name: 'Materiality Analysis Process',
          description:
            'How does the company conduct its materiality analysis, and what criteria are used?',
        },
        {
          id: 'ESRS2-02-02',
          name: 'Stakeholder Engagement',
          description:
            'Which stakeholders are involved in the materiality analysis process, and how is their feedback considered?',
        },
      ],
    },
    {
      id: 'ESRS2-03',
      esrsGroup: 'ESRS 2',
      name: 'Disclosure of Corporate Policies and Goals',
      tags: ['Policies', 'Goals'],
      descriptionForInterviewModerator:
        'Gather information about the company-wide sustainability policies and goals.',
      interviewTopics: [
        {
          id: 'ESRS2-03-01',
          name: 'Corporate Sustainability Policies',
          description:
            'What sustainability policies has the company implemented, and how are they monitored?',
        },
        {
          id: 'ESRS2-03-02',
          name: 'Sustainability Goals and Metrics',
          description:
            'What specific sustainability goals has the company set, and what metrics are used to monitor them?',
        },
        {
          id: 'ESRS2-03-03',
          name: 'Reporting Practices',
          description:
            'How does the company ensure the transparency and completeness of its sustainability reporting?',
        },
      ],
    },
  ]

};

